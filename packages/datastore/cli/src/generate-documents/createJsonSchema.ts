import { getPrimaryKey, ModelDefinition } from "@graphback/core";
import { getNullableType, GraphQLOutputType, isNonNullType } from "graphql";
import { convertToTsType } from "../utils";

const getFieldParameters = (
  fieldName: string,
...
const getModelProperties = (model: ModelDefinition, primaryKey: string) => {
  ...
  const relationships = model.relationships
    ...
    .map((r) => {
      ...
      switch (r.kind) {
        case "oneToMany":
          fieldOptions.type = "(string | undefined)[]";
          return { [`${r.ownerField.name}Ids`]: fieldOptions };
        default:
          // { type: 'string', key: 'nameId' }
          const s = { [relationForeignKey]: fieldOptions };
          return s;
      }
    });
